...
class SalesInvoicePreviewDialog(QDialog):
    def __init__(
        ...
        self.setWindowTitle("پیش نمایش فاکتور فروش")
        self.setModal(True)
        ...
        title = QLabel("پیش نمایش فاکتور فروش")
        title.setStyleSheet("font-size: 16px; font-weight: 700;")
        title.setAlignment(Qt.AlignRight | Qt.AlignVCenter)
        title.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Fixed)
        content_layout.addWidget(title, 0, Qt.AlignRight)

        subtitle = QLabel("لطفاً قبل از ثبت، فاکتور فروش را بررسی کنید.")
        ...
        total_cost_label = self._summary_label(
            "جمع کل", format_amount(data.total_amount)
        )
        ...
        if data.projections:
            ...
            for row_idx, projection in enumerate(data.projections):
                ...
                self._set_item(
                    self.stock_table, row_idx, 2, str(projection.sold_qty)
                )
                self._set_item(
