...
func ensureCoreSchema(ctx context.Context, pool *pgxpool.Pool) error {
	steps := []struct {
		...
		{
			name: "product_group_members table",
			sql: `
		...
		{
			name: "invoice_stock_effects table",
			sql: `
		...
		{
			name: "product group members group index",
			sql:  `CREATE INDEX IF NOT EXISTS idx_product_group_members_group_id ON product_group_members (group_id)`,
		},
		{
